<?php
/**
 * Plugin Name:       Simple Accordion Block
 * Description:       Create beautiful accordions in the Gutenberg editor.
 * Requires at least: 6.6
 * Requires PHP:      7.4
 * Version:           1.0.0
 * Author:            Biplob Ghatak
 * Author URI:        https://ghatakbits.in/
 * License:           GPL v2 or later
 * License URI:       https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain:       simple-accordion-block
 */

// Stop Direct Access 
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}


if( ! class_exists( 'sa_Accordion_Block' ) ) {

	final class sa_Accordion_Block {

		/**
		 * Plugin Version
		 */
		const VERSION = '1.3.6';

		// instance
		protected static $instance = null;

		/**
		 * Constructor
		 */
		public function __construct() {
			
			// define constants
			$this->define_constants();

			// init
			$this->init();

			// enable redirect
			register_activation_hook( __FILE__, [ $this, 'redirect_to_admin' ] );

			// handle redirect
			add_action( 'admin_init', [ $this, 'handle_redirection' ] );
		}

		/**
		 * Define Constants
		 */
		public function define_constants() {
			$constants = [
				'sa_VERSION' => self::VERSION,
				'sa_URL'     => plugin_dir_url( __FILE__ ),
				'sa_PATH'    => plugin_dir_path( __FILE__ ),
				'sa_LIB_URL' => plugin_dir_url( __FILE__ ) . 'includes/',
			];

			foreach ( $constants as $key => $value ) {
				if ( ! defined( $key ) ) {
					define( $key, $value );
				}
			}
		}

		/**
		 * Initialize the plugin
		 */
		public function init() {
			// require_once sa_PATH . 'inc/Admin/Admin.php';
			require_once sa_PATH . 'inc/Plugin/Accordion.php';
		}

		/**
		 * Redirect to admin page after activation
		 */
		public function redirect_to_admin() {
			set_transient( '_sa_redirect', true, 30 );
		}

		/**
		 * Handle Redirection
		 */
		public function handle_redirection() {
			if ( get_transient( '_sa_redirect' ) ) {
				delete_transient( '_sa_redirect' );
				if ( is_admin() && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) && ! ( defined( 'DOING_CRON' ) && DOING_CRON ) ) {
					wp_safe_redirect( admin_url( 'plugins.php' ) );
					exit;
				}
			}
		}

		/**
		 * Instance of the class
		 */
		public static function instance() {
			if ( is_null( self::$instance ) ) {
				self::$instance = new self();
			}
			return self::$instance;
		}

	}

	/**
	 * Initialize the plugin
	 */
	function sa_accordion_block() {
		return sa_Accordion_Block::instance();
	}

	// kick-off
	sa_accordion_block();
}