...
if( ! class_exists( 'sa_Style' ) ) {
    ...
    class sa_Style {
        ...
        public function generate_style( $block_content, $block ) {
            ...
            if ( isset( $block['blockName'] ) && str_contains( $block['blockName'], 'sa/' ) ) {
                
                // Trigger the custom action for rendering the block.
                do_action( 'sa_render_block', $block );
                ...
                if ( ! empty( $block_style ) ) {
                    $handle = 'sa-accordion-style-' . $unique_id;
                ...
                if ( isset( $attributes['accordionBorderRadius'] ) ) {
                    $custom_css .= "{$prefix} .wp-block-sa-accordion-child { border-radius: {$attributes['accordionBorderRadius']}px; }";
                }
                if ( isset( $attributes['accordionActiveBorderColor'] ) ) {
                    $custom_css .= "{$prefix} .wp-block-sa-accordion-child.sa__active_accordion { border-color: {$attributes['accordionActiveBorderColor']} !important; }";
                }
                if ( isset( $attributes['headerActiveBg'] ) ) {
                    $custom_css .= "{$prefix} .wp-block-sa-accordion-child.sa__active_accordion .sa__head { background: {$attributes['headerActiveBg']} !important; }";
                }
                if ( isset( $attributes['headingActiveColor'] ) ) {
                    $custom_css .= "{$prefix} .wp-block-sa-accordion-child.sa__active_accordion .sa__heading_tag { color: {$attributes['headingActiveColor']} !important; }";
                }
                if ( isset( $attributes['bodyActiveBg'] ) ) {
                    $custom_css .= "{$prefix} .wp-block-sa-accordion-child.sa__active_accordion .sa__body { background-color: {$attributes['bodyActiveBg']} !important; }";
                }
                ...
                $custom_css .= "{$prefix} .sa__collapse svg { width: {$icon_size}px; fill: {$inactive_icon_color}; }";
                $custom_css .= "{$prefix} .sa__expand svg { width: {$icon_size}px; fill: {$active_icon_color}; }";
        ...
        */
        public function render_inline_css( $handle, $css ) {
            wp_register_style( $handle, false, array(), sa_VERSION, 'all' );
            wp_enqueue_style( $handle, false, array(), sa_VERSION, 'all' );
        ...
        public function editor_assets() {
            wp_enqueue_style( 'sa-accordion-editor', sa_URL . 'css/editor.css', [], sa_VERSION );
        }
    ...
    sa_Style::instance(); // Initialize the class
