...
if ( ! class_exists( 'sa_Fonts' ) ) {
	...
	class sa_Fonts {
		...
		public function __construct() {
			...
			add_action( 'sa_render_block', array( $this, 'font_generator' ) );
		}
		...
		public function font_generator( $block ) {
			if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
				...
				foreach ( $attributes as $key => $value ) {
					if ( ! empty( $value ) && strpos( $key, 'sa_' ) === 0 && strpos( $key, 'FontFamily' ) !== false ) {
				...
				if ( ! empty( $fonts ) ) {
					...
					if ( ! empty( $gfonts ) ) {
						...
						foreach ( $font_array as $font ) {
							...
							$font_handle = 'sa-font-' . sanitize_title( $font );
	...
	sa_Fonts::instance(); // Initialize the class.
}
